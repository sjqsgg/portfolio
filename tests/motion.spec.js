...
test('full curved transition keeps its duration and Contact has its own layout', async ({ page }) => {
  ...
  expect(effects).toContainEqual({ name:'route-curve-shape', duration:990, delay:560 })
  expect(effects).toContainEqual({ name:'route-finish', duration:190, delay:1570 })
  const textEffects = effects.filter(effect => effect.name === 'route-text-snapshot')
  expect(textEffects).toHaveLength(3)
  expect(textEffects.map(effect => Math.round(effect.delay))).toEqual([970,1045,1120])
