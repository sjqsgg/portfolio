...
test('felt board opens from its surface with margins and closes outside or with Escape',async({page})=>{
  await start(page)
  await expect(page.locator('.workbench-canvas')).toHaveAttribute('data-board-model','felt')
  const p=await point(page,'board')
  await page.mouse.click(p.x+8,p.y+7)
  const dialog=page.getByRole('dialog',{name:'Felt board',exact:true})
  await expect(dialog).toBeVisible()
  await expect(page.locator('.felt-board-canvas')).toHaveAttribute('data-state','ready',{timeout:15000})
  await expect(page.locator('.felt-board-canvas')).toHaveAttribute('data-board-model','felt')
  await page.waitForTimeout(900)
  const box=await dialog.boundingBox()
  expect(box.x).toBeGreaterThan(30);expect(box.y).toBeGreaterThan(30)
  const surface=await page.locator('.felt-board-panel').evaluate(el=>{const css=getComputedStyle(el);return {background:css.backgroundColor,border:css.borderWidth,radius:css.borderRadius,shadow:css.boxShadow}})
  ...
  await page.keyboard.press('Enter');await expect(page.locator('.felt-board-canvas')).toHaveAttribute('data-state','ready')
...
test('mobile close view and board keep a usable frame without visible scene labels',async({page})=>{
  ...
  await page.locator('[data-anchor="board"]').focus();await page.keyboard.press('Enter')
  const dialog=page.getByRole('dialog',{name:'Felt board',exact:true})
  await expect(dialog).toBeVisible();await expect(page.locator('.felt-board-canvas')).toHaveAttribute('data-state','ready')
  await page.waitForTimeout(900)
  const box=await dialog.boundingBox()
